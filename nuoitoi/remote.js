...
break;case -1:this.L$(new g.NA(64));break;case -1E3:this.L$(new g.NA(128,{errorCode:"mdx.remoteerror",errorMessage:"Video n\u00e0y kh\u00f4ng kh\u1ea3 d\u1ee5ng \u0111\u1ec3 ph\u00e1t l\u1ea1i t\u1eeb xa.",xM:2}))}m=lX(this.SG).trackData;var M=this.B;(m||M?m&&M&&m.trackName==M.trackName&&m.languageCode==M.languageCode&&m.languageName==M.languageName&&m.kind==M.kind:1)||(this.B=m,W2C(this,m));m=lX(this.SG);m.volume===-1||Math.round(this.G.getVolume())===m.volume&&this.G.isMuted()===m.muted||this.nE.isActive()||
this.Uw()}else HCd(this)};
...
H2.prototype.UD=function(m){if(this.api.getPresentingPlayerType()===3){var M={RECEIVER_NAME:this.api.getOption("remote","currentReceiver").name};m=g.O(m,128)?g.eb("L\u1ed7i tr\u00ean $RECEIVER_NAME",M):m.isPlaying()||m.isPaused()?g.eb("\u0110ang ph\u00e1t tr\u00ean $RECEIVER_NAME",M):g.eb("\u0110\u00e3 k\u1ebft n\u1ed1i v\u1edbi $RECEIVER_NAME",M);this.updateValue("statustext",m);this.fade.show()}else this.fade.hide()};g.V(RH,g.iv);RH.prototype.Y=function(){var m=this.G.getOption("remote","receivers");m&&m.length>1&&!this.G.getOption("remote","quickCast")?(this.KC=g.UY(m,this.L,this),this.P5(g.ED(m,this.L)),m=this.G.getOption("remote","currentReceiver"),m=this.L(m),this.options[m]&&this.Wf(m),this.enable(!0)):this.enable(!1)};
RH.prototype.L=function(m){return m.key};
RH.prototype.h0=function(m){return m==="cast-selector-receiver"?"Truy\u1ec1n...":this.KC[m].name};
